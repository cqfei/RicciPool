...
class DGCNN(nn.Module):
    def __init__(self, output_dim, num_node_feats, num_edge_feats,
                 ...
                 conv1d_kws=[0, 5],device='cpu',number_iterations=5,lr=1.0,alpha=0.5):
        ...
        ks = [0.9, 0.7, 0.6, 0.5]
        self.Pool = pool.Pool(self.k,device=device,number_iterations=number_iterations,lr=lr,alpha=alpha)#.cuda()

        weights_init(self)

    def forward(self, graph_list, node_feat, edge_feat):
        # graph_ricci_curv = [OllivierRicci(g.g, alpha=0.5, verbose="INFO").compute_ricci_curvature() for g in graph_list]
        graph_sizes = [graph_list[i].num_nodes for i in range(len(graph_list))]
        node_degs = [torch.Tensor(graph_list[i].degs) + 1
                     for i in range(len(graph_list))]
        node_degs = torch.cat(node_degs).unsqueeze(1).to(device=node_feat.device)
        ...
        if isinstance(node_feat, torch.cuda.FloatTensor):
            ...
            n2n_sp = n2n_sp.to(device=node_feat.device)
            e2n_sp = e2n_sp.to(device=node_feat.device)
            subg_sp = subg_sp.to(device=node_feat.device)
            node_degs = node_degs.to(device=node_feat.device)
        ...
        h = self.sortpooling_embedding(
            graph_list, node_feat, edge_feat, n2n_sp, e2n_sp, subg_sp, graph_sizes,
            node_degs,device=node_feat.device,number_iterations=self.number_iterations)
        return h

    def sortpooling_embedding(self, graph_list, node_feat, edge_feat, n2n_sp, e2n_sp,
                              subg_sp, graph_sizes, node_degs,device='cpu',number_iterations=5):
        ...
        if torch.cuda.is_available() and isinstance(node_feat.data, torch.cuda.FloatTensor):
            batch_sortpooling_graphs = batch_sortpooling_graphs.to(device=device)
            batch_sortpooling_As = batch_sortpooling_As.to(device=device)
        
        batch_sortpooling_graphs = Variable(batch_sortpooling_graphs)
        batch_sortpooling_As = Variable(batch_sortpooling_As)
        accum_count = 0

        n2n_dense = self.sparse_to_dense(n2n_sp,device)
        '''  CRF pooling '''
        ...
        for i in range(subg_sp.size()[0]):
            ...
            if self.training:
                A_sparse=torch.sparse_coo_tensor(indices=A.nonzero().t(), values=A[A.nonzero().t()[0], A.nonzero().t()[1]], size=A.size())
                X_out, A_out, f_loss = self.Pool(A_sparse, X_i, U_X, graph_list[i])
            else:
                # X_out, A_out = self.Pool(A,  X_i, U_X, graph_list[i])
                A_sparse = torch.sparse_coo_tensor(indices=A.nonzero().t(),
                                                   values=A[A.nonzero().t()[0], A.nonzero().t()[1]], size=A.size())
                X_out, A_out = self.Pool(A_sparse, X_i, U_X, graph_list[i])
      ...
      #  print("After conv1, the shape is :", conv1d_res.size())
       ...
       # print("After pooling, the shape is :", conv1d_res.size())
        ...
        if self.output_dim > 0:
            ...
            reluact_fp = to_dense
        if self.training:
            return F.relu(reluact_fp), f_loss
        return F.relu(reluact_fp)


    def sparse_to_dense(self, adj,device):
